...
contract NFTMarketplace {
  ...
  function fillOffer(uint _offerId) public payable canFillOffer(_offerId){
  ...
  function cancelOffer(uint _offerId) public canCancelOffer(_offerId){
    _Offer storage _offer = offers[_offerId];    
    nftCollection.transferFrom(address(this), msg.sender, _offer.id);
  ...
  function claimFunds() public {
	...
	userFunds[msg.sender] = 0; // preventing repeated fund transfer through reentrancy attack by making the amount 0 
   	(bool sent, ) = payable(msg.sender).call{value: userFunds[msg.sender]}(""); //use the latest recommended method to transfer ether 
  ...
  modifier canFillOffer(uint _offerId) {
        _Offer storage _offer = offers[_offerId];  
  ...
  modifier canCancelOffer(uint _offerId) {
        _Offer storage _offer = offers[_offerId];  
        require(_offer.offerId == _offerId, 'The offer must exist');
